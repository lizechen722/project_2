...
colnames(k_mean_lasso)[53] <- "OS.event"
colnames(k_mean_lasso)[54] <- "Overall.survival..days."

for (i in 1:nrow(k_mean_lasso)) {
  if (k_mean_lasso$grouping[i] %in% c(1,2,4,5,7,9,10)) {
...
title("Prediction of 3-year survival (Training Set - CAC)")
...
colnames(k_mean_test)[53] <- "OS.event"
colnames(k_mean_test)[54] <- "Overall.survival..days."

for (i in 1:nrow(k_mean_test)){
  if (k_mean_test$grouping[i] %in% c(1,3,7)){
...
title("Prediction of 3-year survival (Testing Set - All calcification)")
AUC_obj_test
...
write.csv(combined_export, "CAC_radiomic_vector.csv", row.names = FALSE)
...
library(PASWR)
library("rms")
radiomic_vector <- read.csv("radiomic_vector.csv")
radiomic_vector <- as.data.frame(radiomic_vector)

nomogram <- merge(clinical_data,radiomic_vector,by = "ID")
...
fit <- lrm(formula = OS.event ~ Age + Stage + Gender, Performance + Tumour + all_calc.y, data = nomogram) 
