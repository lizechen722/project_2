...
colnames(k_mean_lasso)[122] <- "OS.event"
colnames(k_mean_lasso)[123] <- "Overall.survival..days."

for (i in 1:nrow(k_mean_lasso)) {
  if (k_mean_lasso$grouping[i] %in% c(1,2,5)) {
...
title("Prediction of 3-year survival (Training Set - All calcification)")
...
colnames(k_mean_test)[122] <- "OS.event"
colnames(k_mean_test)[123] <- "Overall.survival..days."

for (i in 1:nrow(k_mean_test)){
  if (k_mean_test$grouping[i] %in% c(1,2,3,4,10)){
...
b <- ggsurvplot(
  ...
  palette="jco",
  tables.theme = theme_survminer(font.main = 12),
  title = "Kaplan-Meier Plot Stratified Based on RPV \n (Testing set, All calcification radiomics)",
  xlab="Time in Days",
  ylab="Probability of Overall Survival",
  #surv.median.line = "v",
  ylim=c(0,1),
...
write.csv(combined_export, "all_calc_radiomic_vector.csv", row.names = FALSE)
